...
def linearCorrelation(yPredicted, lkrM, krM, totGen, outFolder, mask_test):
    ...
    hist1, xedges, yedges = np.histogram2d(totGen[myMasks[0]], yPredicted, bins=(80, 80), range=[[340, 1999], [340, 1999]])
    hist2, xedges, yedges = np.histogram2d(totGen[myMasks[1]], lkrM[lkrM>-998], bins=(80, 80), range=[[340, 1999], [340, 1999]])
    hist3, xedges, yedges = np.histogram2d(totGen[myMasks[2]], krM[krM>-998], bins=(80, 80), range=[[340, 1999], [340, 1999]])

    vmin = 1#min(np.min(hist1), np.min(hist2), np.min(hist3))
    vmax = max(np.max(hist1), np.max(hist2), np.max(hist3))
        #cbar_ax = fig.add_axes([0.81, 0.11, 0.05, 0.77])
        #cbar = fig.colorbar(im, cax=cbar_ax)
    ax[0].hist2d(totGen[myMasks[0]], yPredicted, bins=(80, 80), range=[[340, 1999], [340, 1999]] ,norm=mpl.colors.LogNorm(vmin=vmin, vmax=vmax), cmap=plt.cm.jet, cmin=1)
    ax[1].hist2d(totGen[myMasks[1]], lkrM[myMasks[1]], bins=(80, 80), range=[[340, 1999], [340, 1999]] ,norm=mpl.colors.LogNorm(vmin=vmin, vmax=vmax), cmap=plt.cm.jet, cmin=1)
    ax[2].hist2d(totGen[myMasks[2]], krM[myMasks[2]], bins=(80, 80), range=[[340, 1999], [340, 1999]] ,norm=mpl.colors.LogNorm(vmin=vmin, vmax=vmax), cmap=plt.cm.jet, cmin=1)
    corr = []
...
def invariantMass(yPredicted, lkrM, krM, totGen, outFolder):
        print("Invariant Mass plot...")
        lkrM = lkrM[lkrM>-998]
        krM  = krM[krM>-998]
        f, ax = plt.subplots(1, 1)
        if (totGen is not None):
            ax.hist(yPredicted, bins = 80, range=(200,1500), label="DNN   R = %.3f"%(len(yPredicted)/len(totGen)),    histtype=u'step', alpha=0.8, density=True) 
            ax.hist(lkrM,       bins = 80, range=(200,1500), label="Loose R = %.3f"%(len(lkrM)/len(totGen)),  histtype=u'step', alpha=0.5, density=True)
            ax.hist(krM,        bins = 80, range=(200,1500), label="Full  R = %.3f"%(len(krM)/len(totGen)),histtype=u'step', alpha=0.5, density=True)
            ax.hist(totGen,     bins = 80, range=(200,1500), label="True  I = %dk"%(len(totGen)*0.001),   histtype=u'step', alpha=0.8, density=True)
        else:
...
def diffCrossSec(yPredicted, lkrM, krM, totGen, dnnMatrix, looseMatrix, kinMatrix, outFolder, write, recoBin=recoBin ):
        '''
        ...
        ax1.errorbar(x_+dx_     , dnnUnfolded    , yerr=np.sqrt(np.diag(dnnCov)), label='Unfolded DNN',      linestyle=' ', marker='o', markersize = 6, color='C0')
        ax1.errorbar(x_+4/3*dx_ , looseUnfolded  , yerr=np.sqrt(np.diag(looseCov)), label='Unfolded Loose',  linestyle=' ', marker='o', markersize = 6, color='C1')
        ax1.errorbar(x_+2/3*dx_ , kinUnfolded    , yerr=np.sqrt(np.diag(kinCov)), label='Unfolded kin',      linestyle=' ', marker='o', markersize = 6, color='C2')
        ax1.set_xscale('log')
        ax2.set_xscale('log')
        ...
        ax2.plot(x_+dx_     , np.sqrt(np.diag(dnnCov))/dnnUnfolded      ,   label='Unfolded DNN', linestyle='-', marker='o', markersize = 6, color='C0')
        ax2.plot(x_+dx_ , np.sqrt(np.diag(looseCov))/(looseUnfolded), label='Unfolded Loose Err', linestyle='-', marker='o', markersize = 6, color='C1')
        ax2.plot(x_+dx_ , np.sqrt(np.diag(kinCov))/kinUnfolded     , label='Unfolded kin', linestyle='-', marker='o', markersize = 6, color='C2' )
...
def doEvaluationPlots(yTrue, yPredicted, lkrM, krM, outFolder, totGen, mask_test, write):
    ...
    invariantMass(yPredicted=yPredicted, lkrM=lkrM, krM=krM, totGen=totGen, outFolder=outFolder)
...
# *******************************
    ...
    for i in range(len(recoBin)-1):
        ...
        maskGen = np.array((yTrue >= recoBin[i]) & (yTrue<recoBin[i+1]))                # all the generated that satisfy the cuts
...
# for loose and kin masks to be applied to totgen
    for i in range(len(logbins_wu)-1):
        ...
        regMeanBinned       = np.append( regMeanBinned      ,   np.mean(yPredicted[mask]-yTrue[mask]))
        ...
        regSquaredBinned       = np.append( regSquaredBinned      ,   np.mean((yPredicted[mask]-yTrue[mask])**2))
        ...
        regRmsBinned        = np.append( regRmsBinned       ,   np.std(yPredicted[mask]-yTrue[mask]))
        ...
        regErrRmsBinned     = np.append( regErrRmsBinned    ,   np.sqrt((moment(yPredicted[mask]-yTrue[mask], 4)-((regElementsPerBin[i]-3)/(regElementsPerBin[i]-1)*regRmsBinned**4)[0])/regElementsPerBin[i]))
...
# ******************************* 
    ...
    print("DNN matrix")
    dnnMatrix = ResponseMatrix(y=yPredicted, yGen = yTrue, matrixName = 'dnnMatrix', outFolder = outFolder, totGen=totGen)
    ...
    looseMatrix = ResponseMatrix(y=lkrM[lkrM>-998], yGen=totGen[lkrM>-998], matrixName = 'looseMatrix', outFolder = outFolder, totGen=totGen)

    kinMatrix = np.empty((nRecoBin, nRecoBin))
    kinMatrix = ResponseMatrix(y=krM[krM>-998], yGen=totGen[krM>-998], matrixName = 'kinMatrix', outFolder = outFolder, totGen=totGen)
...
# !!! agggiungere un boolean che ti dice se e andato a buon fine e in caso contrario non stampare nessuna matrice
    ...
    diffCrossSec(yPredicted=yPredicted, lkrM=lkrM, krM=krM, recoBin=recoBin, totGen=totGen, dnnMatrix=dnnMatrix, looseMatrix=looseMatrix, kinMatrix=kinMatrix, outFolder=outFolder, write=write )
...
# *******************************
    ...
    index = 0    
    myMasks = [mask_test, (lkrM>-998), (krM>-998)]  
    for el in [yPredicted, lkrM[lkrM>-998], krM[krM>-998]]:
        fig, ax = plt.subplots(1, 1, figsize=(8, 8))
        fig.subplots_adjust(right=0.8)
          
        counts, xedges, yedges, im = ax.hist2d(totGen[myMasks[index]], el - totGen[myMasks[index]], bins=(80, 80), range=[[340, 1500], [-500, 500]] ,norm=mpl.colors.LogNorm(), cmap=plt.cm.jet)
        cbar_ax = fig.add_axes([0.81, 0.11, 0.05, 0.77])
        cbar = fig.colorbar(im, cax=cbar_ax)
...
# *******************************
    ...
    for i, ax in enumerate(axes):
        if i < 7:
            mask = (totGen>recoBin[i]) & (totGen<recoBin[i+1])
            g=sns.kdeplot(data = yPredicted[(yTrue > recoBin[i]) & (yTrue < recoBin[i+1])] - totGen[(mask) & (mask_test)], common_norm=False, fill=True, alpha=.5, linewidth=0, ax=ax, label='DNN', gridsize=5000)
            sns.kdeplot(data = krM[(krM>-998) & mask] - totGen[(krM>-998) & (mask)], common_norm=False, fill=True, alpha=.5, linewidth=0, ax=ax, label='Full', gridsize=5000)
            sns.kdeplot(data = lkrM[(lkrM>-998) & mask] - totGen[(lkrM>-998) & (mask)], common_norm=False, fill=True, alpha=.5, linewidth=0, ax=ax, label='Loose', gridsize=5000)
            ax.set_xlim(-500 ,500)
            ax.set_xlabel(f"X-axis Label {i+1}")
            ax.set_ylabel("Density")
            ax.set_title(r"${} \leq m_{{t\bar{{t}}}} \leq {}$".format(recoBin[i], recoBin[i+1]))
