...
# COVARIANCE MATRIX
    ...
    normalizedMeanSquared = np.sqrt(np.average(((y_predicted[dnnMask_test]-totGen_test[dnnMask_test])**2)/(totGen_test[dnnMask_test]**2), weights = weights_test[dnnMask_test]))
    
    # JS
    p = y_predicted[dnnMask_test]
    q = totGen_test[dnnMask_test]
    x1, x2, nbin = 250, 2000, 100
    ...
    histP = np.histogram(p, weights=weights_test[dnnMask_test], bins=bins)[0]
    histQ = np.histogram(q, weights=weights_test[dnnMask_test], bins=bins)[0]
...
def main():
    maxEvents_          = None
    size = 2**np.random.randint(low=7, high=14) #4
    size=4096
    hp = {
        'batchSize':        size,
        'validBatchSize':   size,
...
# nNode generation
    nDense = np.random.randint(2, 3)        # 1
    nNodes = []
    for idx in range(nDense):
        t = 2**np.random.randint(3, 10)         # 2
        nNodes.append(t)
    ...
    with open("/nfs/dust/cms/user/celottog/mttNN/outputs/comparisons/0708_phase1.txt", "a+") as f:
                string = "%d\t%d\t" %(res.mean(), np.std(res, ddof=1)/np.sqrt(len(res)))
                string = string + str(nNodes) + "\t%.2f"%lasso + "\t%.7f"%ridge+ "\t%.5f"%learningRate + "\t%d"%size+ "\t%.7f"%(mse.mean()) + "\t%.7f"%(np.std(mse, ddof=1)/np.sqrt(len(mse)))  + "\t%.4f"%jd0.mean() + "\t%.4f"%(np.std(jd0, ddof=1)/np.sqrt(len(jd0))) +"\n"
                f.write(string)
